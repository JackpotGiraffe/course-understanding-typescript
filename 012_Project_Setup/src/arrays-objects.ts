...
// --------------------------------------------------------------
let users: (string | number)[]; // union type 也可以用在陣列型別中，array of (string or number)
...
// --------------------------------------------------------------
let arrayUser: Array<string | number>; // 也可以使用泛型 (Generics) 的語法來定義陣列型別
...
// --------------------------------------------------------------
// 如果希望變數只有兩個元素
let possibleResultsAny: number[] // 這樣雖然能達到目的，但無法限制長度
...
// --------------------------------------------------------------
let user: { // 物件有宣告的 properties 和型別，賦值時就必須符合這些宣告
    name: string;
    age: number;
    hobbies: string[];
    role: {
...
let data: Record<string, string | number>; // key 可以是 string 或 number，value 可以是 string 或 number
